...
if input_mode == "CSV questions":
    if not batch_results:
        ...
        with tab_overview:
            ...
            if kept_rows:
                card_map = {(row["batch_id"], row["id"]): row for row in card_rows}
                for row in kept_rows:
                    card_row = card_map.get((row["batch_id"], row["id"]), {})
                    row["resolution_card"] = card_row.get("resolution_card", "")
    ...
    if res is None:
        ...
        with tab_overview:
            ...
            if df_init.empty or df_init_for_download is None:
                ...
                df_merged = df_init_for_download.copy()
                df_merged["resolution_card"] = df_merged["id"].map(
                    lambda qid: card_store.get(qid, {}).get("card", "")
                )
